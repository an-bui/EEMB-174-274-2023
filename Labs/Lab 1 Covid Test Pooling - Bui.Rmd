...
Three other binomial functions in R:  
...
`sign()` is a base function that returns a vector with the sign of the numbers in the input vector as 1 (if the number is positive), 0 (if the number is 0), and -1 (if the number is negative).  
...
z %>%
  Re() %>%  # Take the real part...
  abs() %>% # then take the absolute value...
  sqrt() %>% # modification: take the square root
  log()  # modification: use natural log
...
calc_tests <- function(k) { # function takes the argument k, which is the number of people in each pool
  
  pos_rate <- 0.05 # positive rate
  samps <- 10000 # number of samples
  positive = rbinom(n = samps, size = k, prob = pos_rate)  # Make new data for each run
  pools <-  tibble(positive) # create a tibble
  
  pools <- pools %>%
    mutate(retest = sign(positive) , tests= 1 + retest * k) # figure out if the one test is posiitive and number of tests needed to figure out who is positive
  
  output <- mean(pools$tests) / k # calculate mean tests needed
...
```{r run_function}
calc_tests(3)  # Mean tests needed for when pools have three people each
```
...
**Numbers change with each run (different pool size):** The numbers change with each run when changing the pool size because there are different numbers of individuals in each pool, so the probability of getting at least one person in the pool testing positive changes, and therefore the number of tests needed to retest everyone also changes.  

**Numbers change with each run (same pool size):**  The numbers change with each run because they are random draws from a distribution, so there will not be the same numbers pulled with each run.  

```{r}
positive_1 = rbinom(n = samps, size = 1, prob = pos_rate)  # Make new data for each run
pools_1 <-  tibble(positive_1) %>% 
    mutate(retest = sign(positive) , tests= 1 + retest * 1) 

head(pools_1)
```

**calc_tests(1) not exactly equal to 1:** the output when the pool size is 1 is not exactly equal to 1 because, in this scenario, each individual in the pool is retested whenever someone tests positive. Even if there is only one person in the pool and that person tests positive, the next step is to test them again, meaning that there are scenarios where there are two tests used for one person in a pool.  
...
The pooling strategy uses **fewer tests** than the one-test-per-person strategy. The optimum pool size for the pooling strategy is **5 individuals**.
...
```{r}
keffects_100 <- tibble(k = rep(seq(100), 100)) %>% 
  rowwise() %>%
  mutate(ave_tests = calc_tests(k), poolsize = as.integer(k))

keffects_100_plot <- ggplot(data = keffects_100, aes(x = k, y = ave_tests, group = poolsize)) +
  geom_boxplot()
...
When the pool size is 100, the optimal pool size is still 5, but as pool size increases the average number of tests needed increases to about 1. This is because, as pool size increases, you are more likely to see at least one person in the pool testing positive, and therefore you would have to test everyone in the pool again.
